<header class="header">
    <div class="flex">
        ...
        <div class="icons">
            ...
            <?php
            $count_wishlist_itmes = $conn->prepare("SELECT * FROM wishlist WHERE user_id = ?");
            $count_wishlist_itmes->execute([$user_id]);
            $total_wishlist_items = $count_wishlist_itmes->rowCount();
            ?>
            <a href="wishlist.php" class="cart-btn"><i class="bx bx-heart"></i><sup><?=$total_wishlist_items ?></sup></a>
             <?php
            $count_cart_itmes = $conn->prepare("SELECT * FROM cart WHERE user_id = ?");
            $count_cart_itmes->execute([$user_id]);
            $total_cart_items = $count_cart_itmes->rowCount();
            ?>
